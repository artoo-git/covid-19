...
data<-read.csv(file = "https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv",header = TRUE,sep = ",")
...
xESrelax<-predictdf[which(predictdf$absDay==82 & predictdf$country == "Spain"),][4] %>% as.numeric
yESrelax<-predictdf[which(predictdf$absDay==82 & predictdf$country == "Spain"),][1] %>% as.numeric
...
png("images/Rplot6_D.png", width = 800, height = 800, units = "px")
...
plot1<-ggplot(data = predictdf, aes(x=absDay, y=count, colour=country, breaks = 10)) +
  ...
  geom_segment(mapping=aes(x=xESrelax,   xend=xESrelax,y=0,yend=yESrelax), color = "black", linetype = 9,size = .1)+
  geom_segment(mapping=aes(x=xESrelax,   xend=Inf,y=yESrelax,yend=yESrelax), color = "black", linetype = 9,size = .1)+
  ...
  annotate("text", hjust= 0, x=xESrelax,   y= 0, size=4, angle=90, vjust=-0.4, label="Spain relax measures - 12 April") +
  ...
  labs( title = "Cov-19: count Death cases registred by country (dots)",
...
png("images/daycount_D.png", width = 800, height = 800, units = "px")
ggplot(data = long[which(long$count>50),], aes(x=day, y=daily, colour=country)) +
  ...
  #annotate("text", hjust= 0, x=xUKlockdwn, y= 0, size=4, angle=90, vjust=-0.4, label="UK lockdown") +
  labs( title = paste("GAM approx. of daily death cases as per: ", sysdate),
